...
def test_p15_non_live_multi_project_matrix_is_isolated_and_zero_external() -> None:
    ...
    assert all(
        callable(globals()[project["acceptance_test"]]) for project in projects
    )

    controls = matrix["cross_project_controls"]
    assert controls == {
    ...
    assert all(callable(globals()[case["acceptance_test"]]) for case in cases)
    ...
    acceptance = matrix["acceptance_boundary"]
    assert set(acceptance.values()) == {False}
