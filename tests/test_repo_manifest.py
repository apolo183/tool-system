...
TS_B02A_HOSTED_CAPABILITY_PROBE = (
...
def test_current_repository_manifest_covers_every_tracked_path_once() -> None:
    ...
    assert rows_by_path[TS_B02A_HOSTED_CAPABILITY_PROBE] == {
        "path": TS_B02A_HOSTED_CAPABILITY_PROBE,
        "role": "executable TS-B02A Hosted capability probe suite",
        "purpose": (
            "Probe on GitHub Hosted Ubuntu x86_64, using only bounded synthetic "
            "fixtures, the frozen local-OS isolation prerequisites, observation "
            "completeness, and creator-owned cleanup without implementing "
            "runtime code or executing a real workload."
        ),
        ...
        "downstream": (
            "TS-B02A backend feasibility evidence and separately authorized "
            "implementation planning"
        ),
        "validation": (
            "focused Hosted probe, repository manifest, full tests, unchanged "
            "Hosted CI"
        ),
        "status": "REGISTERED",
    }
...
def test_blueprint_module_registry_packaging_and_ci_register_manifest() -> None:
    ...
    assert blueprint_compiler["module_version"] == "1.0.0"
    assert {
        boundary["path"]
        for boundary in blueprint_compiler["boundaries"]["code"]
    } == {
