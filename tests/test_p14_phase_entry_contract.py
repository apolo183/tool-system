...
REPORT = (
    ROOT / "docs" / "reports" / "p14a_blueprint_to_code_phase_entry_and_contract.md"
)
MANIFEST = (
    ROOT
    / "examples"
...
def test_p14_stage_sequence_closes_each_missing_product_link() -> None:
    ...
    boundaries = {
        stage["stage"]: stage["execution_boundary"] for stage in p14["stage_plan"]
    }
    ...
    assert stages["P14B_PROVIDER_NEUTRAL_AI_WORKER_CONTRACT"]["entry_requires"] == [
        "P14A_PHASE_ENTRY_END_TO_END_SPECIFICATION accepted",
        "explicit P14B source-implementation authorization",
    ]
    assert stages["P14C_BOUNDED_REAL_MODEL_PROVIDER_EXECUTION"]["entry_requires"] == [
...
def test_p14a_manifest_remains_closed_while_p14c_is_exactly_authorized() -> None:
    blueprint = load_yaml_file(BLUEPRINT)
