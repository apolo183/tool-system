...
DIRECT_EFFECT_EXPECTATIONS = {
    ...
    "recovery_planning": frozenset(),
    "operational_observability": frozenset(),
    "record_retention": frozenset(),
    "subscription_capacity": frozenset(),
}
...
def _mappings_by_current_id() -> dict[str, dict[str, Any]]:
    ...
    _require(len(rows) == 25, "module identity mappings must contain twenty-five rows")
    result = {
        str(row["current_module_id"]): row
        for row in rows
        if isinstance(row, dict)
    }
    _require(len(result) == 25, "current module IDs must be unique")
    return result
...
def _validate_contract_set(
    ...
    _require(len(contracts) == 25, "exactly twenty-five module contracts are required")
    for key_path in (
        ("contract_path",),
...
def _validated_contracts() -> tuple[list[dict[str, Any]], list[str]]:
    ...
    _require(actual_paths == expected_paths, "contract directory must contain exactly twenty-five owners")
...
def test_all_module_contracts_match_module_registry_and_real_owner_evidence() -> None:
    ...
    assert len(digests) == len(set(digests)) == 25
