...
The independent optional API plugin proof requires one explicitly enabled and currently usable key to complete one controlled smoke test on the then-canonical main when that plugin is released. It is not a P15 core, P16 entry, or default-route completion gate. It does not require OpenAI and Qwen together, Qwen funding, simultaneous multi-provider availability, or a date-pinned version for a provider that exposes only a moving model ID. Each actual live audit records the provider/model requested and the redacted result.
