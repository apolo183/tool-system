...
The active objective is runtime foundation: consume validated task graphs, assign role steps, run no-mutation role execution records, and prepare audit and rollback bundles while preserving the existing no-mutation target-repository boundary.

## Repository contract

This repository is a tool layer. Business systems are downstream targets. tool-system may propose and apply code changes to target repositories only through explicit workflow, test, policy, and review gates. In P8, target repositories still require separate explicit approval before the first real target-repository mutation.
