...
- read-only verification, packet-only validation, and injected fake-transport tests for the corrected P14C adapter and merged `P14C-LIVE-ISSUER-IMPL-v1` GitHub owner-comment verifier, exact in-memory grant, and exact transport-bound capability now on main, without credential-value access or a real provider call;
