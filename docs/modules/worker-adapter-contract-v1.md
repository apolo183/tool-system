...
isolated workspace. It passes the prompt on stdin, forces non-interactive approval denial, ignores
behavioral user configuration while retaining normal authentication, explicitly
selects an ephemeral read-only sandbox, consumes only a schema-bound final-message
file, and never
...
module_compound_contract:
  ...
  role:
    summary: adapt worker requests into policy-gated orchestration and guarded subscription-worker records with stable safe terminal classifications
    responsibility_boundary: Convert worker requests into adapter requests, keep the deterministic no-mutation adapter as default, expose a separately enabled and explicitly authorized Codex CLI subscription adapter, classify every guarded process outcome with a stable safe terminal code, carry blocked terminals through the development bridge, assemble orchestration and rollback records, and enforce false target-mutation and production flags.
  ...
  side_effect_contract:
    ...
    delegated_effects:
      - capability_id: injected-worker-adapter
        ...
        boundary: The default remains DryRunWorkerAdapter. The guarded Codex CLI adapter separately requires enablement, authorization, fixed argv without a shell, stdin prompt transfer, non-interactive approval denial, ignored user behavioral configuration, minimal credential-name-denying environment, isolated workspace, explicit ephemeral read-only execution, creator-owned private schema and final-message files, timeout with process-group termination, bounded event and final output, strict structured-patch validation, and stable safe terminal classification. The temporary files are removed at the end of their creator-owned lifetime. This conservative maximum does not claim direct Git, database, API-provider, target-repository, remote-repository, or production effects and grants no authority.
        classification_grants_authority: false
    classification_grants_authority: false
  compatibility_policy:
    interface_compatible_replacement: Preserve request and result fields, optional stable terminal-code addition without changing default dry-run records, blocked bridge terminal propagation, guarded subscription-adapter enablement and authorization, shell-free fixed argv, stdin prompt, non-interactive approval denial, ignored user behavioral configuration, explicit ephemeral read-only sandbox, minimal environment, finite prompt, process, event, and final-result limits, process-group cancellation, schema-bound structured-result-only output, no-target-mutation flags, orchestration parity, rollback bundle, policy gate, and audit shapes.
  ...
  replacement_contract:
    activation_rule: Replace only after adapter contract, worker conversion, unchanged default no-mutation records, guarded subscription preflight, fake-process stdin/argv/approval/config-isolation/environment/private-schema/final-message/output/timeout/stable-terminal/process-group-cancellation, blocked bridge envelope, strict structured-patch, orchestration, nested-policy, audit-write, and rollback tests pass.
  ...
  external_system_contracts:
    ...
    systems:
      - system_id: codex-cli-subscription-worker
        ...
        evidence_paths:
          - src/tool_system/worker_adapter/contract.py
        boundary: Spawn the owner-configured Codex executable only through a shell-free fixed argument vector with non-interactive approval denial, explicit exec, JSONL events, ephemeral mode, ignored user behavioral configuration, read-only sandbox, creator-owned output schema and final-message paths, and stdin prompt; use a minimal environment that rejects provider-credential variable names, explicit enablement and authorization, an isolated workspace, bounded prompt/event/final bytes, timeout with finite process-group TERM-to-KILL cancellation, strict structured-patch validation, stable safe terminal codes, and no raw-output, prompt-argument, or temporary-path retention.
  non_claims:
