...
mapping_contract:
  ...
  mappings:
    ...
    - current_module_id: ai_worker_runtime
      ...
      change_risk: "critical: the default remains fixture-only, P14C retains its grant-bound proof, and P15C uses separate repository-external per-installation settings and credentials, source-sealed owner-only controls, private target transfer gates, atomic budget accounting, fixed direct-TLS OpenAI and DeepSeek routes, Hosted fake I/O only, Qwen disabled, and redacted public evidence"
