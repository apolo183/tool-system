...
`docs/model_provider_portfolio_and_economics_contract_v1.md` owns the detailed roadmap contract. Every provider-specific adapter requires fake-I/O contract coverage, while the overall API backup path requires only one explicitly enabled and currently usable key to complete one controlled smoke test on the then-canonical main. Simultaneous multi-provider availability, Qwen funding, and date-pinned versions for moving provider aliases are not completion gates. P14 remains the provider-neutral blueprint-to-code core plus its accepted bounded historical provider proof. P15 owns multi-project evidence, adapter contracts, failure/economics evidence, and the final single-provider smoke gate. P16 owns sustainable operations, with API portfolio operations conditional on API enablement. This principle grants no live provider, credential, target-repository, phase-entry, or production authority.
