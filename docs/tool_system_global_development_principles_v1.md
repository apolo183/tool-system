...
Before every important engineering task, consume the current `main` branch
directly from the canonical central remote
`git@github.com:apolo183/finance-governance.git`, using only the fixed formal
paths `docs/global_development_principles_v1.md` and
...
Central rules prevail on conflict; this file may add only tool-system-specific
constraints. Central principles are consumed at source rather than copied here,
and tool-system state is not written back to the identity-only central registry.
...
`config/module_registry_v1.yaml` is the machine-readable inventory of current tool-system modules and public dependency declarations. Its four-field top-level shape is owned by tool-system. The local validator enforces exact module/interface identities, tracked natural-owner and repository-local boundary coverage without overlap, ContractReference SHA-256 values, managed Python import edges against declared consumers, side-effect target bindings, dependency-version reciprocity, and an acyclic declared graph. This structural and source-graph evidence is not evidence of runtime isolation, behavioral interface compatibility, or automatic replacement.
...
Declared module-graph structure, exact tracked ownership and repository-local boundaries, managed Python import-edge derivation, ContractReference hashes, and effect-target bindings now have local machine validation. Runtime module isolation, task planning from the graph, blueprint compilation, behavioral interface compatibility evidence, fault-isolation impact records, automated replacement, multi-project acceptance, benchmarks, and production review remain future tool-system responsibilities. Local registry validation is not sufficient for final product acceptance: P14E owns compiler and runtime-flow integration, and P15 owns separately authorized multi-project evidence. None of those stages receives downstream write authority from this file.

## 25. Provider portfolio, model routing, and development economics

Supported ChatGPT/Codex subscription mode is the ordinary development route. It is distinct from every metered API and must not be treated as API credit or implemented by scraping authentication material. All large-model APIs, including OpenAI, DeepSeek, Qwen, and future providers, are dormant backup routes and are disabled by default. A key's presence never grants authority or enables a call.

Provider integrations are replaceable adapters behind a versioned worker interface. Provider-specific credentials, billing, transport, retry behavior, and data policy must not leak into the task planner or unrelated modules. The public repository stores credential references, schemas, disabled examples, redacted fixtures, and policy only; it never stores secret values, browser session material, private billing records, private operating-cost values, or a mandatory provider/model selection. Repository-external operator configuration must explicitly enable API mode and one provider, select the provider/model available at that time, and set budget, expiry, transfer, retry, cancellation, and audit controls.
...
Provider unavailability and output-quality failure are distinct. Unconfigured, unfunded, invalid, expired, or unavailable providers may be skipped. Missing credentials, quota, balance, rate limit, timeout, or outage may trigger bounded failover only to another already enabled, eligible, and authorized route; the replacement need not be stronger. Quality failure may receive bounded same-route repair and then evidence-backed escalation to a stronger eligible route. Policy denial, unauthorized data transfer, stale precondition, hard budget exhaustion, or missing required evidence blocks execution and cannot be bypassed through provider switching. If API mode is enabled but no route remains, the runtime returns a stable no-available-provider result.

Models may be qualified per task class rather than by one global ranking when API mode is enabled. Discovery does not activate a model and version number does not prove superiority. Public source does not require a provider to expose a date-pinned version or bind project completion to a moving alias. The actual provider/model requested is recorded in redacted execution evidence. Promotion, demotion, degradation, and retirement of reusable enabled routes use current price, policy, health, and benchmark evidence; a repeatedly failing model is removed from affected task classes while unrelated routes remain available.
...
Provider and economics maintenance is conditional on API mode being explicitly enabled. Discovery, health/price refresh, incremental benchmarks, portfolio review, and renewal review use repository-external operator-configured cadence; none is a default-off project-completion gate. Severe outages, security advisories, material price changes, or regressions still trigger review for enabled routes. Exact private currency values, cadence, and weights remain installation configuration outside the public repository.

`docs/model_provider_portfolio_and_economics_contract_v1.md` owns the detailed roadmap contract. Every provider-specific adapter requires fake-I/O contract coverage. The independent optional API backup plugin requires only one explicitly enabled and currently usable key to complete one controlled smoke test on the then-canonical main when that plugin is released; that proof may be deferred and is not a P15 core, P16 entry, or default-route completion gate. Simultaneous multi-provider availability, Qwen funding, and date-pinned versions for moving provider aliases are not completion gates. P14 remains the provider-neutral blueprint-to-code core plus its accepted bounded historical provider proof. P15 owns multi-project core evidence, adapter contracts, and failure/economics evidence. P16 owns sustainable operations, with API portfolio operations conditional on API enablement. This principle grants no live provider, credential, target-repository, phase-entry, or production authority.
